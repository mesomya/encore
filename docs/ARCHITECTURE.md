...
How Refeed is put together â€” the execution contexts, the data flows, the deliberate
decisions, and the gotchas. Read this before making non-trivial changes.

## The big picture

Refeed is a Manifest V3 extension with **three execution contexts** plus a popup, talking
across worlds:

```
 â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ x.com tab â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”         â”Œâ”€â”€ extension â”€â”€â”
 â”‚  MAIN world           ISOLATED world     â”‚         â”‚  service      â”‚
 â”‚  page-hook.js  <â”€â”€â”€â”€> content.js  <â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â–ºâ”‚  background.js â”‚
 â”‚  (sees X's fetches)   (bridge + weave)   â”‚  chrome â”‚  (IndexedDB)   â”‚
 â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  msgs   â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
        â–² window.postMessage  â–² chrome.runtime              â–² chrome.runtime
        â””â”€â”€â”€ cross-world â”€â”€â”€â”€â”€â”˜                       â”Œâ”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”
                                                      â”‚ popup/*   â”‚
                                                      â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```

Why these contexts? The **request recipe** is learned in the background via `webRequest`
(network-layer, read-only) â€” the reliable path that doesn't depend on page internals.
...
### `src/page-hook.js` â€” page MAIN world, `document_start`
...
- Also wraps `window.fetch` **and** `XMLHttpRequest` as a *secondary* capture path
  (`classify()` â†’ `captureTemplate()` â†’ `TEMPLATE`/`HARVEST`). The *primary* capture is
  `webRequest` in the background (below), so this is belt-and-braces.
- Idempotent: guards `window.__REFEED_PAGE_HOOK__` so a double injection can't double-wrap.
- Uses **no `chrome.*` APIs** â€” it's immune to "Extension context invalidated".

### `src/content.js` â€” ISOLATED world, `document_start`
...
- **Collect orchestration.** `pageSync()` (RPC to the page hook) and `runSync()` (the
  manual collect the popup triggers â€” incremental, reports progress). A 700 ms route poll
  (`onRoute`) only resets the Home weaving cadence. Collection is **manual-only**: there is
  no auto-collect, top-up, or on-page toast.
- Idempotent: guards `window.__REFEED_CONTENT__` against double injection.

### `src/background.js` â€” service worker
...
- **Primary request capture.** A read-only `webRequest` listener (`onBeforeRequest` +
  ...
  persists a replayable template via `storeTemplate` â€” the reliable, cross-browser capture
...
- **Existing-tab injection.** `injectIntoOpenTabs()` uses `chrome.scripting` to attach both
  ...
  tab open before the extension loaded still works â€” no forced reload.
- Message handlers: `STORE_TWEETS` (upsert; merges `sources`), `GET_MIX_BATCH` (reservoir
  ...
  uses â€” drives a logged-in tab), `SYNC_COMPLETE`/`SYNC_FAILED`/`finishSync`,
...
**Capture / "teach" (first-run).** User opens Bookmarks/Likes â†’ X fetches that timeline â†’
...
patch is a secondary path to the same `STORE_TEMPLATE`, plus `HARVEST` â†’ `STORE_TWEETS`.)

**Collect (popup button).** `START_SYNC` â†’ `startSync` messages a logged-in tab `RUN_SYNC`
â†’ content `runSync` â†’ `pageSync` sends `SYNC` (with the persisted template + `knownIds`) â†’
page-hook `runSync` replays paginated â†’ `PAGE` events â†’ content `STORE_TWEETS` â†’ upsert.
**Incremental:** stops as soon as a whole page is already in `knownIds` (newest-first â‡’
caught up). Empty archive â‡’ full walk.

**Weave (feed injection).** content `MutationObserver` â†’ `considerCell` (every Nth real
tweet) â†’ `injectInto` â†’ `GET_MIX_BATCH` â†’ `buildCard` appended **inside** the host
...
- **`classify()` substring matching, with guards.** Match by substring to survive X
  renaming ops (`Bookmarks`, `BookmarksAllInOne`, `UserLikes`â€¦). Three guards exist
  because of real bugs:
  - `folder` â†’ `null` (first check): `BookmarkFoldersSlice` is the folder *list* (no
    posts) and was overwriting the real `Bookmarks` template â†’ collect fetched 0.
...
- **Pixel-native card.** Set X's exact font stack explicitly (inheritance rendered the
  ...
  "You reposted" row â€” no Refeed branding on the post.
- **Incremental collect** with a forced-full escape hatch (Empty the library, then
  Collect â€” an empty archive does a full walk).
- **Capture at the network layer.** Learning the request recipe via `webRequest` in the
  background is immune to page CSP, injection timing, and which thread X uses â€” the
...
- **Manual-only collection.** Collecting runs solely from the popup's Collect buttons â€”
  no route-poll auto-collect, no background top-up, no on-page toast â€” so behavior is
  predictable and nothing surprises the user.

## Gotchas (these will bite you)

- **You can't intercept the page's `history.pushState` from a content script** â€” the
  ...
  content script does affect the real URL â€” that's why `openInApp` works.)
- **`chrome.runtime.sendMessage` throws *synchronously*** ("Extension context
  invalidated") if the extension is reloaded/updated while a tab stays open â€” a trailing
  `.catch()` can't stop it. All contentâ†’background messaging goes through `bg()`, which
...
- **Content scripts only inject into pages loaded *after* the extension is enabled.** A
  tab that was already open gets nothing until it reloads â€” which is why the background
  calls `injectIntoOpenTabs()` (`chrome.scripting`) on install / startup / worker wake to
  attach both worlds to already-open x.com tabs. (Capture itself doesn't depend on this â€”
  `webRequest` sees the request regardless.)
- **`requestAnimationFrame` is paused on hidden/unfocused tabs** â€” count-up animations can
...
- **UI** (cards/pill/popup) can be verified headless with a throwaway HTML page that loads
  ...
  blocked by drivers). The real TwitterChirp font only exists on x.com â€” judge layout, not
  the font.
...
User-facing words are Refeed's own: **Collected / Liked / Saved**, **Replay in my feed**,
