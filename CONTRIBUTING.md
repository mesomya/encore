...
3. **Load unpacked** → select the `refeed` folder.

After editing, click the **↻ reload** button on the Refeed card in `chrome://extensions`,
...
- **Capture/collect changes** need a logged-in X session — there's no way around it.
  Use the `[Refeed] …` `console.info` breadcrumbs in the page console to see what's
...
- Any `[Refeed]` lines from the **page** console, and any errors from the **service
  worker** console (`chrome://extensions` → Refeed → *Inspect views: service worker*).
